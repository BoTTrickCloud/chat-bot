...
async def bot(request, *args):
    ...
    if data["action"]:
        async with request.config_dict["db"].connect() as conn:
            result = await conn.execute(
                sql.select([Qas.c.answer]).where(
                    sql.and_(
                        (Qas.c.static.is_(True)), (Qas.c.question == data["question"])
                    )
                )
            )
            qas = await result.fetchone()
        return web.json_response(
            {"response": qas.answer, "actions": [], "language": data["language"]}
        )
    ...
    async with request.config_dict["db"].connect() as conn:
        result = await conn.execute(
            sql.select([Qas.c.question, Qas.c.answer]).where(
                sql.and_((Qas.c.static.is_(False)), (Qas.c.id == int(idxs[0][0])))
            )
        )
        qas = await result.fetchone()
    ...
    if distance > int(request.config_dict["config"].DISTANCE_THRESHOLD):
        logger.warning(
            f"Distance: {distance}, ClientQ: {data['question']}, "
            f"Q: {question}, A: {answer}"
        )
        return web.json_response(
            {
                "response": "Sorry, I didn't find any valid answer. "
                            "Please try to ask again or contact us by form.",
                "actions": [
                    {"action": "contact_form", "value": "Contact Form"},
                    {"action": "about_me", "value": "About Me"},
                ],
    ...
    logger.info(
        f"Distance: {distance}, ClientQ: {data['question']}, Q: {question}, A: {answer}"
    )
    return web.json_response(
        {"response": answer, "actions": [], "language": data["language"]}
    )
...
async def qas_get(request, *args):
    ...
    async with request.config_dict["db"].connect() as conn:
        data_res = await conn.execute(
            sql.select(columns).where(Qas.c.static.is_(False))
        )
...
async def qas_put(request, *args):
    ...
    async with request.config_dict["db"].connect() as conn:
        ...
        id_res = await conn.execute(
            sql.select([Qas.c.id]).where(Qas.c.vector == vector)
        )
